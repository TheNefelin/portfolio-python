...
@router.get("/pagination", response_model=dtos.PaginationResponse[dtos.TechnologyResponse])
...
@router.get("/", response_model=list[dtos.TechnologyResponse])
async def get_all(db: AsyncSession = Depends(get_db)):
  result = await service.get_all(db, page=1, limit=100)
  return result.items


@router.get("/{id}", response_model=dtos.TechnologyResponse)
...
@router.post("/", response_model=dtos.TechnologyResponse, status_code=status.HTTP_201_CREATED)
async def create(data: dtos.TechnologyRequest, db: AsyncSession = Depends(get_db)):
  return await service.create(db, data)


@router.put("/{id}", response_model=dtos.TechnologyResponse)
...
@router.post("/{id}/upload-image", response_model=dtos.TechnologyResponse)
