...
async def get_by_id(db: AsyncSession, id: int) -> dtos.LanguageResponse:
...
async def update(db: AsyncSession, id: int, data: dtos.LanguageRequest) -> dtos.LanguageResponse:
  entity = await repository.get_by_id(db, id)
  if not entity:
    raise NotFoundError("Language")
...
async def upload_image(db: AsyncSession, id: int, file_bytes: bytes) -> dtos.LanguageResponse:
  entity = await repository.get_by_id(db, id)
  if not entity:
    raise NotFoundError("Language")
  if entity.img_url:
